...
class PlayerClass {
    ...
    public StrengthAdd: number;
    public DefenseAdd: number;
    public EnergyAdd: number;
                        ...
                        LoadArea(UI.AtWork, function () {
                            ...
                            $('#atWorkProgressBar').progressTimer({
                                ...
                                eachIteration: function (elapsed) {
                                    ...
                                    if (price < 1) $('#atWorkArtifactPrice').text('1');
                                    else $('#atWorkArtifactPrice').text(price);
                                },
                                onFinish: function () {
                                    $('#atWorkBtn').empty();
...
// UI
function LoadArea(ui: UI, callFunction?, type = 0, parameter?) {
    if (game.UI != UI.HeroQuest) {
        if (game.UI != UI.AtNarrativeQuest) {
            if (game.UI != UI.AtWork) {
                if (game.UI != UI.City) { CloseArea(); }
                ...
                if (parameter != null) { request += '&param1=' + parameter; }
...
function LoadIndexArea(partialName, divName, append?, callFunction?, parameter?) {
    ...
    if (append == false || append == null) {
        ...
        $.get(request, function (data) { $(divName).after(data); if (callFunction != null) { callFunction(); } });
    }
}
...
function PurchaseItem(itemId) {
    if (game.UI == UI.Shop) {
        $.ajax({
            ...
            success: function (data) {
                $.each(data, function (key, val) {
                    if (val.Success == "true") { NotificationAlert(val.Title, val.Msg, NotificationType.Success); }
                    else { NotificationAlert(val.Title, val.Msg, NotificationType.Alert); }
                    GameUpdateGold(); GameUpdateInventoryWeight(); GameUpdateInventoryBricABrac();
                });
            },
        ...
        $.ajax({
            ...
            success: function (data) {
                $.each(data, function (key, val) {
                    ...
                    GameUpdateArtifacts(); GameUpdateInventoryWeight(); GameUpdateInventoryBricABrac();
                });
            },
            error: function (XMLHttpRequest, textStatus, errorThrown) {
                NotificationAlert("Error", "Internal Error: " + errorThrown, NotificationType.Alert);
...
function ConvertArtifactToGold() {
    if (game.UI == UI.Shop) {
        $.ajax({
            ...
            success: function (data) {
                $.each(data, function (key, val) {
                    if (val.Success == "true") { NotificationAlert(val.Title, val.Msg, NotificationType.Success); GameUpdateGold(); GameUpdateArtifacts(); }
                    else { NotificationAlert(val.Title, val.Msg, NotificationType.Alert); }
                });
            },
            error: function (XMLHttpRequest, textStatus, errorThrown) {
    ...
    $.ajax({
        ...
        success: function (data) {
            $.each(data, function (key, val) {
                if (val.Success == "true") {
                    NotificationAlert(val.Title, val.Msg, NotificationType.Success);
                    GameUpdateCharacter(); GameUpdateGold();
                }
                else { NotificationAlert(val.Title, val.Msg, NotificationType.Alert); }
            });
        },
...
function PurchaseCompanion(itemId) {
    $.ajax({
        ...
        success: function (data) {
            $.each(data, function (key, val) {
                if (val.Success == "true") {
                    NotificationAlert(val.Title, val.Msg, NotificationType.Success); GameUpdateGold();
                }
                else { NotificationAlert(val.Title, val.Msg, NotificationType.Alert); }
            });
        },
            ...
            if (val.Step == 1) { // Timer
                $('#heroQuestDivCenter').progressTimer({
                    ...
                    eachIteration: function (elapsed) {
                        ...
                        if (price < 1) { $('#atHeroQuestArtifactPrice').text('1'); }
                        else { $('#atHeroQuestArtifactPrice').text(price); }
                    },
                    onFinish: function () {
                        $('#heroQuestDivCenter').empty();
                        $('#heroQuestDivCenter').html('<a onclick="HeroQuestNextStep(2)" class="btn btn-space btn-primary">' + val.BtnNextStep + '</a>');
                ...
                GameUpdateGold(); GameUpdateLife(); GameUpdateCharacter();
                CloseArea();
            }
        });
        game.GetPlayer().Update();
        ...
        $.each(result, function (key, val) {
            if (val.Step == 0) { // Timer
                $('#heroQuestDivCenter').progressTimer({
                    ...
                    eachIteration: function (elapsed) {
                        var price = Math.ceil((((val.Time * 1000) - elapsed) / 900000) * 3);
                        if (price < 1) { $('#atNarrativeQuestArtifactPrice').text('1'); }
                        else { $('#atNarrativeQuestArtifactPrice').text(price); }
                    },
                    onFinish: function () {
                        $('#heroQuestDivCenter').empty();
            ...
            else if (val.Step == -10) { // Get Reward
    ...
    $.ajax({
        ...
        success: function (data) {
            $.each(data, function (key, val) {
                ...
                else { NotificationAlert(val.Title, val.Msg, NotificationType.Alert); GameUpdateCompanion(); }
            });
        },
        error: function (XMLHttpRequest, textStatus, errorThrown) {
            NotificationAlert("Error", "Internal Error: " + errorThrown, NotificationType.Alert);
    ...
    $.ajax({
        ...
        success: function (data) {
            $.each(data, function (key, val) {
                if (val.Success == "true") {
                    NotificationAlert(val.Title, val.Msg, NotificationType.Success);
                    GameUpdateInventoryEquipment(); GameUpdateInventoryWeight(); GameUpdateInventoryBricABrac(); GameUpdateCharacter();
                }
                else { NotificationAlert(val.Title, val.Msg, NotificationType.Alert); }
            });
        },
